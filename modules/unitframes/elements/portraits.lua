...
local HasFemaleHumanPortrait = function(unit)
	...
	local name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellId, canApplyAura, isBossDebuff
	for i = 1,40 do
...
local Update = function(self, event, ...)
	...
	-- The model widget is its own little 3D viewport, not a transparent
	-- overlay - it stays opaque even when it has nothing to render, so it
	-- has to be explicitly hidden (not just left blank) for a 2D portrait
	-- behind it to actually show. UnitIsVisible is exactly the point past
	-- which the 3D model stops being able to render at all (well before
	-- the unit would ever actually leave the group), so it's what decides
	-- which of the two to show - same threshold this element already
	-- used to just hide the portrait outright before Portrait2D existed.
